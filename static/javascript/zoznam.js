...
function editallfilterList() {
    let input, filter, ul, li, a, i;
    input = document.getElementById('editallfilterInput');
    filter = input.value.toUpperCase();
    ul = document.getElementById('editallselectList');
    li = ul.getElementsByTagName('li');
    for (i = 0; i < li.length; i++) {
        a = li[i].getElementsByTagName("a")[0];
        if (a.innerHTML.toUpperCase().indexOf(filter) > -1) {
...
if (document.getElementById("delsButton")) {
...
if (document.getElementById("role")) {
    document.getElementById("role").onclick = function () {
        ...
        document.getElementById('editallfilterInput').value = '';
...
document.querySelectorAll('.pravaButton').forEach(button => {
    button.addEventListener('click', function() {
        ...
        document.getElementById('editfilterInput').value = '';
        sendRequest('/getPozicie', 'GET', null, (data) => {
